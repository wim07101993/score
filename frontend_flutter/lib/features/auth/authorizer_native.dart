...
class PlatformAuthorizer implements Authorizer {
  ...
  Future<Callback?> authorize(Uri authorizationUrl) async {
    ...
    return _read(Uri.parse(answer).queryParameters);
  }

  /// Waits on a port of this machine for the browser to be sent back to it.
  ///
  /// This is the desktop half, and it is done here rather than by
  /// `flutter_web_auth_2` on purpose. That package hangs a lifecycle observer
  /// off the sign-in and tears the whole thing down the moment the app is
  /// resumed — which is right on a phone, where being resumed means the user
  /// came back having dismissed the browser. On a desktop the browser is
  /// another window on the same screen, so the app is resumed the instant it
  /// gets the focus back, which is seconds after the browser opens and long
  /// before anybody has typed a password. The sign-in cancelled itself every
  /// time.
  ///
  ...
  /// The first request that is actually the provider answering.
  ///
  /// A browser asks for more than it was sent to — a favicon, most often — and
  /// none of that is the answer, so it is turned away rather than taken for
  /// one.
  Future<Callback?> _firstAnswer(HttpServer server) async {
    await for (final request in server) {
      ...
      // The browser is in front by now, and what the player wanted was the app.
  ...
  /// How long a sign-in is given before the port is given up on. Long enough to
