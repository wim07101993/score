...
function _drawSetControls() {
  ...
  setButton.textContent = set.title.trim() === '' ? 'Untitled set' : set.title;
  setPosition.textContent = `${index + 1} of ${set.entries.length}`;
  setEntryDescription.textContent = entry.description ?? '';

  // Going back from a song of a gig means going back to the gig.
  backButton.href = setButton.href;
  backButton.setAttribute('aria-label', 'Back to the set');
...
function _pointAt(button, index) {
  ...
  button.href = `perform.html?${new URLSearchParams({
...
function _drawScoreTitle() {
  ...
  scoreTitle.textContent = getScoreTitle(score);
  document.title = getScoreTitle(score);
}
